    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k == 'user':
                ...
                entities_model = getattr(api.parser.model_factory, 'entities')
                entities = entities_model.parse(api, v)
                setattr(status, 'entities', entities)
            elif k == 'created_at':
...
class Entities(Model):
    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k == 'urls':
                setattr(entities, k ,Urls.parse_list(api,v))
            else:
